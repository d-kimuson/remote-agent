...
export const ProjectListPage: FC = () => {
  ...
  const { data: projectsData, refetch: refetchProjects } = useSuspenseQuery({
  ...
  const handleLoadTree = () => {
    loadTreeMutation.mutate(treeRootPath);
  };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 md:px-6">
        <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="space-y-1 text-sm text-muted-foreground md:text-right">
            <p>Working directory: {appInfoData.workingDirectory}</p>
            <p>Projects file: {appInfoData.projectsFilePath}</p>
          </div>
        </header>
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder={appInfoData.workingDirectory}
                  value={treeRootPath}
                />
                <Button
